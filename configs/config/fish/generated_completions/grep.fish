...
complete -c grep -s V -l version --description 'Output the version number of  grep and exit. SS "Pattern Syntax".'
complete -c grep -s E -l extended-regexp --description 'Interpret  PATTERNS as extended regular expressions (EREs, see below).'
complete -c grep -s F -l fixed-strings --description 'Interpret  PATTERNS as fixed strings, not regular expressions.'
complete -c grep -s G -l basic-regexp --description 'Interpret  PATTERNS as basic regular expressions (BREs, see below).'
complete -c grep -s P -l perl-regexp --description 'Interpret PATTERNS as Perl-compatible regular expressions (PCREs).'
complete -c grep -s e -l regexp --description 'Use  PATTERNS as the patterns.'
complete -c grep -s f -l file --description 'Obtain patterns from R FILE , one per line.'
complete -c grep -s i -l ignore-case --description 'Ignore case distinctions in patterns and input data, so that characters that …'
complete -c grep -l no-ignore-case --description 'Do not ignore case distinctions in patterns and input data.'
...
complete -c grep -s z --description '( --null-data ) option, and grep -P may warn of unimplemented features.'
